...
describe('PokemonService', () => {
  ...
  it('Should throw PokemonNotFound exception when getPokemonInfo() returns 404', async () => {
    nock(baseURL).get('/pokemon/test-pokemon-name').reply(404);

    await expect(service.getPokemonInfo('test-pokemon-name')).rejects.toThrowError(
      'Pokemon cannot be found: test-pokemon-name',
    );
  });

  it('Should throw Error when getPokemonInfo() returns 500', async () => {
    nock(baseURL).get('/pokemon/test-pokemon-name').reply(500);
